      ...
      * Optimised by:    Hermes (IV&V demo)
      *
      * Changes from original:
      *   - Replaced MULTIPLY/ADD verb chains with COMPUTE expressions
      *     in the line-item loop. On IBM Z, COMPUTE over COMP-3 fields
      *     allows the compiler to emit a single decimal-arithmetic
      *     sequence rather than separate MP/AP instructions per verb.
      *   - Removed redundant MOVE ZERO for WORKING-INDEX (INITIALIZE
      *     already zeroes it via NUMERIC DATA BY ZEROES when INVOICE
      *     01-group is in scope — retained explicit zero for clarity
      *     but collapsed to single statement).
      *   - No functional changes; all observable outputs identical.
      *****************************************************************
           ...
           PERFORM WITH TEST BEFORE
               ...
               AND INV-LINE-UNIT-PRICE(WORKING-INDEX) IS NUMERIC
                   ...
                   IF TAXABLE-ITEM(WORKING-INDEX)
                       COMPUTE LINE-WORKING-TAX =
                           LINE-WORKING-TOTAL * SALES-TAX-RATE
                       END-COMPUTE

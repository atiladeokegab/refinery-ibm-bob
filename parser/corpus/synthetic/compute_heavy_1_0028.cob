       IDENTIFICATION DIVISION.
       PROGRAM-ID. CMPTH0028.
       AUTHOR. ZOPTIMA-GEN.
       ENVIRONMENT DIVISION.
       INPUT-OUTPUT SECTION.
       FILE-CONTROL.
           SELECT INFILE  ASSIGN TO 'INPUT.DAT'
               ORGANIZATION IS SEQUENTIAL.
           SELECT OUTFILE ASSIGN TO 'OUTPUT.DAT'
               ORGANIZATION IS SEQUENTIAL.
       DATA DIVISION.
       WORKING-STORAGE SECTION.
       01  WS-VAR-000        PIC 9(5) VALUE ZERO.
       01  WS-VAR-001        PIC 9(7)V99 VALUE ZERO.
       01  WS-VAR-002        PIC 9(3) VALUE ZERO.
       01  WS-VAR-003        PIC 9(7)V99 VALUE ZERO.
       01  WS-VAR-004        PIC X(20) VALUE ZERO.
       01  WS-VAR-005        PIC 9(7)V99 VALUE ZERO.
       01  WS-VAR-006        PIC 9(3) VALUE ZERO.
       01  WS-VAR-007        PIC X(20) VALUE ZERO.
       01  WS-VAR-008        PIC 9(7)V99 VALUE ZERO.
       01  WS-VAR-009        PIC 9(7)V99 VALUE ZERO.
       01  WS-VAR-010        PIC X(20) VALUE ZERO.
       01  WS-VAR-011        PIC 9(7)V99 VALUE ZERO.
       01  WS-VAR-012        PIC X(20) VALUE ZERO.
       01  WS-VAR-013        PIC 9(3) VALUE ZERO.
       01  WS-VAR-014        PIC X(20) VALUE ZERO.
       01  WS-VAR-015        PIC 9(5) VALUE ZERO.

       PROCEDURE DIVISION.
       MAIN-LOGIC.
                  COMPUTE WS-VAR-000 = WS-VAR-001 - 36.
                  COMPUTE WS-VAR-001 = WS-VAR-002 * 34.
                  COMPUTE WS-VAR-001 = 23.
                  COMPUTE WS-VAR-002 = WS-VAR-003 * 93.
                  COMPUTE WS-VAR-002 = 47.
                  COMPUTE WS-VAR-003 = WS-VAR-004 + 44.
                  COMPUTE WS-VAR-003 = 33.
                  COMPUTE WS-VAR-004 = WS-VAR-005 - 45.
                  COMPUTE WS-VAR-004 = 60.
                  COMPUTE WS-VAR-005 = WS-VAR-006 + 2.
                  COMPUTE WS-VAR-006 = WS-VAR-007 + 33.
                  COMPUTE WS-VAR-006 = 75.
                  COMPUTE WS-VAR-007 = WS-VAR-008 * 69.
                  COMPUTE WS-VAR-007 = 55.
                  COMPUTE WS-VAR-008 = WS-VAR-009 + 92.
                  COMPUTE WS-VAR-009 = WS-VAR-010 - 71.
                  COMPUTE WS-VAR-010 = WS-VAR-011 + 26.
                  COMPUTE WS-VAR-011 = WS-VAR-012 + 10.
                  COMPUTE WS-VAR-012 = WS-VAR-013 + 8.
                  COMPUTE WS-VAR-013 = WS-VAR-014 - 49.
                  COMPUTE WS-VAR-014 = WS-VAR-015 * 76.
                  COMPUTE WS-VAR-014 = 69.           STOP RUN.

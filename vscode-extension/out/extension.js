...
function getConfig() {
    ...
    return {
        serverUrl: cfg.get('serverUrl', 'http://localhost:8080').replace(/\/$/, ''),
        username: cfg.get('username', 'admin'),
        password: cfg.get('password', 'refinery'),
        estate: cfg.get('enableEstateAnalysis', true),
    };
}
async function apiPost(url, body, token) {
    return new Promise((resolve, reject) => {
        ...
        const req = lib.request({ hostname: parsed.hostname, port: parsed.port, path: parsed.pathname, method: 'POST', headers }, (res) => {
            let raw = '';
...
async function runAudit(filePath, workspacePath) {
    const cfg = getConfig();
    ...
    let result;
    try {
        result = await apiPost(`${cfg.serverUrl}/api/audit/run`, { repo_path: workspacePath, changed_file: relativePath, estate: cfg.estate }, token ?? undefined);
    }
    catch {
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('refinery.runAudit', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('refinery.openLastContract', async () => {
        if (!lastRefId) {
